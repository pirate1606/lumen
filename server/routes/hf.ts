...
export async function handleSymptomsChat(req: Request, res: Response) {
  try {
    ...
    const { prompt, model = "mistralai/Mistral-7B-Instruct-v0.2" } =
      req.body ?? {};
    ...
    try {
      ...
      const out =
        Array.isArray(arr) && arr[0]?.generated_text
          ? arr[0].generated_text
          : t;
...
export async function handleEmbed(req: Request, res: Response) {
  try {
    ...
    if (!key)
      return res.status(500).json({ error: "HF_API_KEY not configured" });
    const { text, model = "sentence-transformers/all-MiniLM-L6-v2" } =
      req.body ?? {};
    ...
    try {
      ...
      return res.json({
        embedding: vec,
        dim: Array.isArray(vec) ? vec.length : undefined,
      });
    } catch {
